...
type DatapointSettings struct {
	...
	Datasource interface{}  `json:"datasource"` // either a Ttnv3Datasource or a WebDatasource or a MqttDatasource or a ParametersDatasource depending on SourceType
}
...
func (ds *ParametersDatasource) UnmarshalUDT(name string, info gocql.TypeInfo, data []byte) error {
	if name == "parameters" {
		var result map[string]string
		json.Unmarshal(data, &result)
		ds.Parameters = result
	}
	return nil
}
...
const (
	...
	Parameters SourceType = "parameters" // Parameters
)
